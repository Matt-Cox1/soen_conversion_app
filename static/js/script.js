    ...
    document.querySelectorAll('.constant-value, .parameter-item span:nth-child(2), .result').forEach(el => {
        ...
        if (el.textContent.trim() && !el.querySelector('.copy-btn')) {
            ...
            copyBtn.addEventListener('click', function(e) {
                ...
                // Extract number in scientific notation from LaTeX output
                const originalText = el.innerHTML;
                let scientificNotation;
                
                // Look for the pattern: digits × 10^{exponent}
                const match = originalText.match(/(\d+\.\d+|\d+)\s*\\times\s*10\^\{([+-]?\d+)\}/);
                if (match) {
                    ...
                    // Fallback to plain text extraction if pattern not found
                    scientificNotation = el.textContent.replace(/.*=\s*/, '').trim();
                }
